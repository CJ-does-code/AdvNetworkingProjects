...
def encrypt_data(rsa_public_key, data):
...
def decrypt_data(rsa_private_key, encrypted_data):
    encrypted_aes_key = encrypted_data[:rsa_private_key.key_size // 8]
    iv = encrypted_data[rsa_private_key.key_size // 8: rsa_private_key.key_size // 8 + 16]
    encrypted_message = encrypted_data[rsa_private_key.key_size // 8 + 16:] 
...
def start_peer_udpserver(private_key_path,peer_ip, peer_port, certfile, cafile,name):
    ...
    while True:
        ...
        sign, address = serverSocket.recvfrom(4096) 
        ...
        try:
            ...
            pem_data = keys[index].encode('utf-8')
            senders = serialization.load_pem_public_key(pem_data)
            try:
                senders.verify(sign,decrypted_message,padding.PKCS1v15(),hashes.SHA256())
            except Exception as e:
                print(f"bad signiture {e}")
            print(f"{dec}")
            response_message = name+"||pong"
   ...
   # Perform mutual TLS handshake
    ...
    try:
        ...
        if text == "register":
            ...
            recipient_public_key = serialization.load_pem_public_key(pubkey_bytes)
            final= encrypt_data(recipient_public_key,response_json)
            ssl_conn.sendall(final)
...
def update_server(peer_ip, peer_port, certfile, keyfile, cafile):
    ...
    received_data = secure_client_socket.recv(8192)
    decrypted_message = decrypt_data(private_key,received_data)
    decoded_data = decrypted_message.decode('utf-8')
