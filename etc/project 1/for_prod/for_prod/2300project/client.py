...
def client():
    ...
    threading.Thread(target=networkfunction.start_peer_udpserver, args=(encoded_key,args.network, 8080, cert, "myCA.crt",args.name,)).start()
    ...
    while True:
        time.sleep(10)
        registered_names,public_keys=networkfunction.update_server(args.network, 8080, cert, encoded_key, "myCA.crt")
        print(f"found {registered_names}")
        time.sleep(5)
